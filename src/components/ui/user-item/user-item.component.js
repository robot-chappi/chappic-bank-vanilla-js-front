...
export class UserItem extends ChildComponent {
	...
	update({ avatarPath, name }) {
		if (avatarPath && name) {
			$C(this.element).find('img').attr('src', avatarPath).attr('alt', name)

			$C(this.element).find('span').text(name)
		}
	}
	...
	render() {
		...
		$C(this.element).click(this.onClick || this.#preventDefault.bind(this))

		if (!this.onClick) $C(this.element).attr('disabled', '')
		if (this.isGray) $C(this.element).addClass(styles.gray)
